...
# Sensible sigma for Gaussian kernel: ~1/2 median inter-label distance
...
class SNRmixDataset(Dataset):
    def __init__(self, img_paths, genes, dist_matrix, proc,
                 alpha=ALPHA, sigma=SIGMA, k=K_NEIGHBORS, include_self=True,
                 cache_cap: int = 0):
        ...
        # (B) precompute kNN indices
        k_eff = self.k if self.include_self else self.k + 1
        ...
        if not self.include_self:
            ...
            for i, row in enumerate(nn_idx):
                r = row.copy()
                if i in r:
                    jpos = int(np.where(r == i)[0][0])
                    r[0], r[jpos] = r[jpos], r[0]
                else:
                    d = self.D[i, r]
                    far = np.argmax(d)
                    r[far] = i
    ...
    def __getitem__(self, idx):
        ...
        j = int(np.random.choice(len(neigh), p=p_k))
        j = int(neigh[j])
        ...
        # return xi, yi, xj, yj, lam, idx, j, and the edge weight w_ij
...
SAVE_ROOT = "/home/hongyi/project/mixup/model/TENX13_random/SNRmix"
LOSS_ROOT = "/home/hongyi/project/mixup/loss_recording/TENX13_SNRmix"
...
for epoch in range(1, EPOCHS + 1):
    ...
    for batch in train_loader:
        ...
        # single forward pass for all three inputs
...
checkpoint = torch.load(os.path.join(SAVE_ROOT, "best_model.pt"))
...
df.to_csv(os.path.join(LOSS_ROOT, "Cmixup_knn.csv"), index=False)

# Persist test metrics as JSON
import json
